...
@section('content')
    <div class="mx-auto max-w-4xl space-y-8">
        <div class="flex items-center justify-between">
            <h2 class="text-3xl font-bold tracking-tight text-foreground">Create New Post</h2>
                ...
                <form action="{{ route('admin.posts.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <p class="text-[10px] text-muted-foreground mt-1">Upload an image (JPG/PNG) OR generate one. AI images are not saved until you click Create Post.</p>
                        ...
                        <div class="flex items-center space-x-2 pt-8">
                            <input type="checkbox" name="is_published" id="is_published" value="1" {{ old('is_published') ? 'checked' : '' }} class="h-4 w-4 rounded border-primary text-primary focus:ring-primary">
                            <label class="text-sm font-medium leading-none" for="is_published">Publish Immediately</label>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4 pt-4 border-t border-border">
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            <div class="space-y-2">
                                ...
                                <input type="text" name="meta_title" id="meta_title" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50" value="{{ old('meta_title') }}">
                            </div>
                            <div class="space-y-2">
                    ...
                    <div class="flex justify-end pt-4">
                        <button type="submit" class="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 bg-primary text-primary-foreground hover:bg-primary/90 h-10 px-4 py-2">Create Post</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        ...
        async function generateContent() {
            ...
            btn.disabled = true;
            btn.innerHTML = '<span class="loading loading-spinner loading-sm mr-2"></span> Generating...'; // Show spinner + text
        ...
        async function generateImage() {
            ...
            try {
                ...
                if (data.image_base64) {
                    const base64Str = "data:image/jpeg;base64," + data.image_base64; // Assuming JPEG/PNG from model
